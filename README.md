...
par un identifiant déjà connu. L'utilisateur colle des triplets
« code INSEE, adresse, nouvel ID erreur » ; le programme en déduit tout le
reste — sans aucun appel réseau, le code INSEE étant fourni directement.
...
L'utilisateur saisit, une ligne par correction, **trois informations** : le
code INSEE de la commune, l'adresse, et la valeur qu'elle doit prendre dans
la colonne « ID erreur » :

```
06027 ; 101 CHEMIN DES GROS BUAUX 06800 CAGNES SUR MER ; 06027_1164
06027 ; 48 CHEMIN DES NOISETIERS 06800 CAGNES SUR MER ; 06027_544
04245 ; 1025 VOIE SOUS CANAL EDF 04130 VOLX ; 04245_17
04245 ; 1025 VOIE SOUS CANAL EDF 04130 VOLX ; 04245_32
```

Le séparateur accepté entre les trois membres est un point-virgule (`;`),
une tabulation (pour un copier/coller depuis trois colonnes Excel) ou une
flèche (`->` ou `=>`) — toujours le même sur une ligne. Le code INSEE est pris
sur le **premier** séparateur rencontré et la valeur d'ID erreur sur le
**dernier** : l'adresse, au milieu, peut donc elle-même contenir un point-
virgule sans perturber l'analyse. Une éventuelle ligne d'en-tête
(« Code INSEE » / « Adresse » / « ID erreur », copiée avec les données depuis
Excel) est ignorée silencieusement.

À partir de la saisie, le programme :

1. **extrait** les triplets valides — une ligne sans deux séparateurs, un
   code INSEE qui n'a pas la forme attendue (5 chiffres, ou `2A`/`2B` + 3
   chiffres), ou une adresse/valeur manquante, est ignorée et signalée ;
2. **regroupe** les triplets par commune → `{ "06027": [...], "04245": [...] }` ;
3. **déduit** le département de chaque code INSEE (`Dep06`, `Dep04`, `Dep2A`) et
   ouvre `<racine>/Dep<XX>/<insee>/audit_*.xlsx` — un seul fichier par
   commune, quel que soit le nombre d'adresses demandées pour elle ;
4. **modifie** uniquement les lignes dont la colonne **« Adresse »** porte
...
l'utilisateur n'a pas saisi ses propres corrections.
...
- **Une même adresse saisie plusieurs fois, avec des valeurs différentes**
  ...
  à corriger — comme `04245_17` et `04245_32` ci-dessus) : les lignes du
  classeur qui portent cette adresse reçoivent les valeurs saisies **dans
  l'ordre** — la 1re ligne rencontrée (de haut en bas dans le fichier) reçoit
  la 1re valeur saisie pour cette adresse, la 2e ligne la 2e valeur, etc.
  S'il y a plus de lignes que de valeurs saisies, la dernière valeur saisie
  est réutilisée pour les lignes excédentaires.
...
compris dans le texte : `12 adresse(s) → 4 commune(s)`, et signale les lignes
incomplètes (code INSEE invalide, adresse ou ID erreur manquant).
...
```
🚀 5 adresse(s) sur 2 commune(s)
📂 Racine : D:\audit_SNA
🔎 Filtre : colonne « Adresse » → écriture dans « ID erreur »  ·  mise en forme reprise de la colonne
────────────────────────────────────────────────────────────
   · 06027 — recherche du fichier audit…
   · 06027 — audit_06027.xlsx (12,4 Mo)
   · 06027 — analyse du fichier…
   · 06027 — 3 ligne(s) à modifier — chargement du classeur…
   · 06027 — application des modifications…
   · 06027 — enregistrement de 3 modification(s)…
✅ 06027 — 3 modification(s) sur 3 ligne(s) · 3/3 adresses trouvées · 4,1 s
   · 04245 — recherche du fichier audit…
   · 04245 — analyse du fichier…
   · 04245 — aucune ligne concernée sur 8 200 lue(s) — non ouvert en écriture
⚠️ 04245 — 0 modification(s) sur 0 ligne(s) · 0/2 adresses trouvées · 0,2 s
   ↳ introuvables : 1025 VOIE SOUS CANAL EDF 04130 VOLX
```
...
│   │   ├── adresses.py         #   saisie → triplets (INSEE, adresse, ID erreur), regroupement
...
graphique. Le programme ne fait **aucun appel réseau** : le code INSEE de
chaque adresse est fourni directement par l'utilisateur.
...
vide, valeur numérique, adresse dupliquée dans le fichier et dans la saisie,
adresse absente, colonne Adresse partiellement vide…) :
...
- Si plusieurs lignes d'un fichier portent la même adresse, toutes sont
  modifiées (cf. « Une seule opération, volontairement » pour la distribution
  des valeurs quand la saisie en propose plusieurs).
